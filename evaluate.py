...
v=data["Close"]>data["mv_avg"]
v_bh=np.ones(v.shape[0])

v_tune = data["Close"] > data["mv_avg"] * a + b
...
npdata=np.array(data["Close"])
npdatamean=np.array(data["mv_avg"])
plt.close()
plt.figure(figsize=(30,10))

plt.plot(npdata, label="Close")
plt.plot(npdatamean,label="mv_avg")
